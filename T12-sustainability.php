<?php
/**
 * Template Name: T12 : CSR and Sustainability
...
<?php
$sustainability_gallery = get_field('sustainability_gallery');
if($sustainability_gallery):
?>
<section id="galleryCSR" class="padding-bottom--100">
    <div class="sustainableGaleery">
        <div class="slider-sustanable-gallery slider">
            <?php foreach($sustainability_gallery as $sustanable_gallery):
                $sustanability_name = $sustanable_gallery['sustainability_gallery_image_name'];
                $sustanability_image = $sustanable_gallery['sustainability_image'];
                $sustanability_image = ($sustanability_image) ? $sustanability_image : validateImageData($sustanability_image, 710, 550, get_the_title());
                $sustanable_hm_cdn_param = $sustanable_gallery['suatainable_gal_image_param'];
                $hp_sustanable_img_url = (!empty($sustanability_image['url']) ? $sustanability_image['url'] : 'hhttps://placehold.co/710x550');
                $hp_sustanable_img_url .= '?w=710&h=550' . (!empty($sustanable_hm_cdn_param) ? '&' . $sustanable_hm_cdn_param : '');
            ?>
                <div>
                    <a class="d-flex flex-column position-relative " title="Image Gallery" href="<?php echo esc_url($hp_sustanable_img_url); ?>" data-fancybox="gallery">
                        <div class="imagegal-sustanable position-relative">                        
                            <img class="large" src="<?php echo esc_url($hp_sustanable_img_url); ?>" alt="<?php echo esc_attr($sustanability_image['alt']); ?>">
                            <div class="inerDetails position-absolute">
                                <p class="sustanablename font-family--opensans font-color--white paragraph--16 padding-bottom--30"><?php _e($sustanability_name, "KingsPavilion");?></p>
                            </div>                        
                        </div>
                    </a>
                </div>
    ...
    <div class="wrapper--80">
        ...
        <div class="sidebysideBlock d-flex justify-content-between align-items-center padding-bottom--100 protab-flex-column mobile-flex-column">
            ...
            <div class="half-div--46 descriptionBlock">
                ...
                <div class="descriptiontext paragraph--20 font-family--opensans font-color--text-color <?php if($select_data_text){ echo 'padding-bottom--30' ;}?> mobile-text-center tab-text-center protab-text-center"><?php _e($side_block_description, "KingsPavilion");?></div>
                <?php
